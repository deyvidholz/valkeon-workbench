  ...
  return (
    <Modal onClose={cloning ? () => {} : close} width={520} zIndex={56} panelStyle={{ padding: 20 }}>
      ...
      <input
        ...
        disabled={cloning}
      />
      {error && <div style={{ fontSize: 11.5, color: '#e07a6e', marginBottom: 16 }}>{error}</div>}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 9 }}>
        <Hover as="span" onClick={cloning ? undefined : close} style={{ padding: '8px 14px', borderRadius: 8, color: '#9a9aa3', fontSize: 12.5, fontWeight: 500, cursor: cloning ? 'default' : 'pointer' }} hover={{ background: '#16161c' }}>
          Cancel
        </Hover>
        <Hover as="span" onClick={cloning ? undefined : doClone} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 15px', borderRadius: 8, background: 'var(--accent)', color: '#0a1018', fontSize: 12.5, fontWeight: 600, cursor: cloning ? 'default' : 'pointer', opacity: cloning ? 0.7 : 1 }} hover={cloning ? undefined : { filter: 'brightness(1.08)' }}>
