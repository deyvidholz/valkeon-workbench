    ...
    <Modal onClose={close} width={440} zIndex={55} panelStyle={{ maxHeight: '82%' }}>
      <div style={{ padding: '15px 18px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>Labels</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {board.labels.map((l) => (
            <Hover key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '8px 9px', borderRadius: 8 }} hover={{ background: 'var(--surface)' }}>
              ...
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: 10.5, color: 'var(--text-faint)', fontFamily: "'Geist Mono', monospace" }}>{usage(l.id)} cards</span>
          ...
          {board.labels.length === 0 && <div style={{ fontSize: 12, color: 'var(--text-faint)', padding: 8 }}>No labels yet.</div>}
        </div>
      </div>

      <div style={{ flexShrink: 0, borderTop: '1px solid var(--line)', background: 'var(--bg)', padding: '14px 18px' }}>
        <div style={{ fontSize: 10.5, color: 'var(--text-muted)', letterSpacing: '0.06em', marginBottom: 9 }}>NEW LABEL</div>
