    ...
    <Modal onClose={close} width={760} zIndex={64} panelStyle={{ maxHeight: '86%' }}>
      <div style={{ padding: '15px 18px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>{t('diagramBuilder.title', 'Diagram builder')}</span>
        </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6, padding: '11px 18px', borderBottom: '1px solid var(--line)' }}>
        <span style={{ fontSize: 11, color: 'var(--text-muted)', marginRight: 2 }}>{t('diagramBuilder.templates', 'Templates:')}</span>
        {TEMPLATES.map((tpl) => (
          <Hover key={tpl.label} as="span" onClick={() => setDiagText(tpl.src)} style={{ padding: '5px 11px', borderRadius: 7, background: 'var(--surface)', border: '1px solid var(--line-2)', color: 'var(--text-2)', fontSize: 11.5, fontWeight: 500, cursor: 'pointer' }} hover={{ background: 'var(--surface-2)', border: '1px solid var(--line-2)' }}>
            {t('diagramBuilder.tpl' + tpl.label, tpl.label)}
          </Hover>
        ))}
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', height: 380 }}>
        <div style={{ width: 300, flexShrink: 0, borderRight: '1px solid var(--line)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ padding: '9px 14px', fontSize: 10.5, color: 'var(--text-muted)', letterSpacing: '0.06em', borderBottom: '1px solid var(--line)' }}>{t('diagramBuilder.mermaidSource', 'MERMAID SOURCE')}</div>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0, background: 'var(--bg)' }}>
          <div style={{ padding: '9px 14px', fontSize: 10.5, color: 'var(--text-muted)', letterSpacing: '0.06em', borderBottom: '1px solid var(--line)' }}>{t('diagramBuilder.preview', 'PREVIEW')}</div>
      ...
      <div style={{ flexShrink: 0, borderTop: '1px solid var(--line)', background: 'var(--bg)', padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 9 }}>
        <Hover as="span" onClick={close} style={{ padding: '8px 14px', borderRadius: 8, color: 'var(--text-dim)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer' }} hover={{ background: 'var(--surface-2)' }}>{t('diagramBuilder.cancel', 'Cancel')}</Hover>
        <Hover as="span" onClick={insert} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 15px', borderRadius: 8, background: 'var(--accent)', color: 'var(--on-accent)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }} hover={{ filter: 'brightness(1.08)' }}>
          <Icon name="check" size={16} />{t('diagramBuilder.insert', 'Insert diagram')}
        </Hover>
      </div>
    </Modal>
  )
