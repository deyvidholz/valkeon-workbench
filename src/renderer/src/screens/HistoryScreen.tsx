    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ padding: '20px 24px 13px', borderBottom: '1px solid var(--line)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em' }}>History</div>
            <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginTop: 3 }}>Everything you do in {project?.name ?? 'this project'}</div>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 24px 28px', minHeight: 0 }}>
        {items.length === 0 ? (
          <div style={{ minHeight: '60%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-dim)' }}>No activity yet. Sessions, terminals, and board changes show up here.</div>
          </div>
        ) : (
          <>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', color: 'var(--text-muted)', padding: '4px 12px 4px' }}>RECENT ACTIVITY</div>
            {items.map((h) => (
              <Hover key={h.id} onClick={() => open(h)} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 12, borderRadius: 9, cursor: h.target ? 'pointer' : 'default' }} hover={{ background: 'var(--surface)' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text-faint)', fontFamily: "'Geist Mono', monospace", flexShrink: 0 }}>{rel(h.ts)}</span>
              </Hover>
            ))}
          </>
        )}
